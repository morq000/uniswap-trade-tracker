...
interface UserConfig {
    USER_CONFIG: {
        ...
        MAX_DUPE_BUY: string;
        COPY_BUY_PERCENT: string;
        DEADLINE: string;
        PORTFOLIO_FILE_PATH: string;
        SLIPPAGE: string;
        ABORT_IF_STATS_FAIL: boolean;
...
const loadConfig = (): UserConfig => {
    if (fs.existsSync(configPath)) {
        ...
        throw new Error('! Отсутствует файл настроек ./user_config.json');
    }
};
const USER_CONFIG = loadConfig().USER_CONFIG;
...
// Проскальзывание
export const SLIPPAGE_PERCENT = new Percent(
    Number(USER_CONFIG.SLIPPAGE),
    10_000
); // делитель 10_000, т.е. 50 это 0.005
...
export const BASE_CURRENCIES = USER_CONFIG.BASE_CURRENCIES;
