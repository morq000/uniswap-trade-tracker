...
export interface Position {
    ...
    amount: string;
    extraBuyTimes: number;
    profitUSD: number;
}

export interface PositionQuery {
    openPosition: Position | undefined;
    openReversePosition: Position | undefined;
}

interface Portfolio {
    openPositions: { [pair: string]: Position };
    closedPositions: { [pair: string]: Position };
    totalProfitUSD: string;
    transactions: Transaction[];
}

export interface ProcessTransactionStatsRequest {
    txHash: string;
    tokenInName: string;
    tokenOutName: string;
    amountIn: bigint;
    ...
    decimalsIn: bigint;
    decimalsOut: bigint;
    tokenInPrice?: Big.Big;
    tokenOutPrice?: Big.Big;
}
...
const getTokenPriceUSD = async (tokenName: string): Promise<number> => {
    for (let i = 0; i < FETCH_PRICE_API_RETRIES; i++) {
        try {
            ...
            console.error(
                `Ошибка при получении цены токена ${tokenName}:`,
                error
            );
        }
    }
    console.error(
        `Не удалось получить цену токена ${tokenName} после ${FETCH_PRICE_API_RETRIES} попыток`
    );
    return 0;
};
...
export const processTransactionStats = async (
    statsData: ProcessTransactionStatsRequest
) => {
    const {
        txHash,
        tokenInName,
    ...
    const id = ethers.utils.id(
        `${timestamp}-${tokenInName}-${tokenOutName}-${amountIn}-${amountOut}`
    );

    // Преобразуем входные значения из bigint в Big с учётом десятичных знаков
    const bigAmountIn = new Big(amountIn.toString()).div(
        new Big(10).pow(Number(decimalsIn))
    );
    const bigAmountOut = new Big(amountOut.toString()).div(
        new Big(10).pow(Number(decimalsOut))
    );
    ...
    if (!priceInUSD) priceInUSD = tokenInPrice ? tokenInPrice.toNumber() : 0;
    let priceOutUSD = await getTokenPriceUSD(tokenOutName);
    if (!priceOutUSD) priceOutUSD = tokenOutPrice ? tokenOutPrice.toNumber() : 0;
    ...
    const existingPosition =
        portfolio.openPositions[pair] || portfolio.openPositions[reversePair];
    let profitUSD: Big.Big = new Big("0");
