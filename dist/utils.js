...
import { DEADLINE, COPY_BUY_PERCENT, TARGET_WALLET_ADDRESS, } from "./config.js";
import { IERC20 } from "./abi/IERC20.js";
export const generateDeadline = () => {
    const currentTimestamp = Math.floor(Date.now() / 1000);
    return currentTimestamp + DEADLINE;
};
export const makeTokenApprove = async (tokenAddress, routerAddress, signer, spendingAmount) => {
    console.log(`Approve called with params: `, tokenAddress, routerAddress, signer, spendingAmount);
...
export const calcBuyPercent = (amountString) => {
...
export const calcSellPercent = async (provider, myAddress, tokenAddress, amountIn // amountIn уже в wei или наименьших единицах актива
) => {
    console.log(`calcSellPercent params: ${myAddress}, ${tokenAddress}, ${amountIn}`);
    // Создаем контракт токена
    const targetBalanceAfterSell = await getCurrencyBalance(tokenAddress, TARGET_WALLET_ADDRESS, provider);
    const targetBalanceBeforeSell = ethers.BigNumber.from(targetBalanceAfterSell).add(amountIn);
    // Получаем баланс нашего кошелька (в wei или наименьших единицах актива)
    const myBalanceBeforeSell = await getCurrencyBalance(tokenAddress, myAddress, provider);
...
export const getCurrencyBalance = async (tokenAddress, ownerAddress, provider) => {
    const tokenContract = new ethers.Contract(tokenAddress, IERC20, provider);
    return await tokenContract.balanceOf(ownerAddress);
};
export const getPriceInUSD = (buyAmount, quoteAmount) => {
