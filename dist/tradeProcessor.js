...
export const processTrade = async (swap) => {
    try {
        ...
        // Проверка баланса
        const currentBalance = await getCurrencyBalance(tokenIn.address, signer.address, provider);
        ...
        if (tokenIn.address.toLowerCase() ==
            ...
            if (nativeBalance <
                ...
                if (BigNumber.from(currentBalance) <
                    BigNumber.from(processedData.amountIn.toString())) {
        ...
        const receiveNativeEther = tokenOut.address.toLowerCase() ==
            WETH_ADDRESS[CHAIN_ID].toLowerCase();
        const tokenOutStartBalance = receiveNativeEther
            ? BigInt((await provider.getBalance(signer.address)).toString())
            : await getCurrencyBalance(tokenOut.address, signer.address, provider);
        const receipt = await executeTrade(tokenIn, processedData.amountIn, tokenOut, sendNativeEther, receiveNativeEther);
        if (receipt && receipt.status == 1) {
            const tokenOutEndBalance = receiveNativeEther
                ? BigInt((await provider.getBalance(signer.address)).toString())
                : await getCurrencyBalance(tokenOut.address, signer.address, provider);
...
const processPositions = async (tokenIn, amountIn, tokenOut) => {
    console.log("ProcessPositions called with params:", tokenIn.symbol, amountIn, tokenOut.symbol);
    ...
    if (tokenIn.symbol && tokenOut.symbol) {
        ...
        console.log(`Got open positions for pair: ${tokenIn.symbol}/${tokenOut.symbol}: `, openPositions);
        if (openPositions.openReversePosition) {
            const sellAmount = await calcSellPercent(provider, signer.address, tokenIn.address, amountIn.toString());
        ...
        if (openPositions.openPosition &&
            openPositions.openPosition.extraBuyTimes >= MAX_DUPE_BUY) {
            console.log(`! Отмена покупки ${tokenOut.symbol}: больше нельзя докупать (${MAX_DUPE_BUY} раза)`);
            return {
                shouldSwap: false,
        ...
        console.log(`Покупка ${tokenOut.symbol}, количество: ${tokenIn.symbol}:${buyAmount}`);
        return {
            shouldSwap: true,
            amountIn: buyAmount,
...
const processData = (swap) => {
    ...
    if ("pair" in swap) {
        ...
        tokenIn =
            swap.amount1In === "0"
                ? new Token(CHAIN_ID, token0.id, Number(token0.decimals), token0.symbol)
                : new Token(CHAIN_ID, token1.id, Number(token1.decimals), token1.symbol);
        tokenOut =
    ...
    if ("origin" in swap) {
        tokenIn = swap.amount1.startsWith("-")
            ? new Token(CHAIN_ID, swap.token0.id, Number(swap.token0.decimals), swap.token0.symbol)
            : new Token(CHAIN_ID, swap.token1.id, Number(swap.token1.decimals), swap.token1.symbol);
        tokenOut =
    ...
    if (!(tokenIn && tokenOut && amountIn)) {
        throw new Error(`Ошибка формирования входных аргументов для свопа: ${JSON.stringify(swap)}`);
    }
